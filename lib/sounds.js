var defaultSequence = Array.apply(null, Array(8)).map(Number.prototype.valueOf,0); // array of 8 zeros for a default sequence
...
// Using the Web Audio API to map our Mongo sounds to sources
Source = function(audioCtx, audioElement, masterSource) {
    ...
    for(var node in masterSource) {
        this.source.connect(masterSource[node]);
    }
}
